        ...
        while (null != targetType)
        {
            ...
            for (int i = 0; i < allFields.Length; ++i)
            {
                ...
                var injectAssetAttribute =
                        fieldInfo.GetCustomAttribute(_injectAssetAttributeType) 
                        as InjectAssetAttribute;
                if (injectAssetAttribute == null)
                    continue;
                
                var objectToInject = context.GetObjectOfType(fieldInfo.FieldType.BaseType, 
                    injectAssetAttribute.AssetName);

                if (null == objectToInject)
                    continue;
